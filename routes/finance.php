...
use App\Http\Controllers\Roles\Cashier\Screening\ScreeningCompanionListController;
use App\Http\Controllers\Roles\Cashier\MedicalRecord\CashierMedicalRecordController;
use App\Http\Controllers\Roles\Cashier\Screening\ScreeningConsultationListController;
...
Route::prefix('dashboard/cashier')->middleware(['role:cashier'])->group(function () {
  ...
  Route::get('companion', [ScreeningCompanionListController::class, 'index'])->name('companion.screening');
  Route::get('consultation', [ScreeningConsultationListController::class, 'index'])->name('cashier.consultation');
  ...
  Route::post('notifications/mark-all-read', function () {
    $user = auth()->user();
    \App\Models\Notifications\Notification::where('user_id', $user->id)
      ->whereNull('read_at')
      ->update(['read_at' => now()]);
